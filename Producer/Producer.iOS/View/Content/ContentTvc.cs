...
namespace Producer.iOS
{
	...
	public partial class ContentTvc : UITableViewController
	{
		...
		{
			if (allAssets?.Count == 0 && ProducerClient.Shared.AvContent.Count > 0)
			{
				allAssets = ProducerClient.Shared.AvContent [UserRoles.General].Where (m => m.HasId && m.HasStorageUrl)
																				 .Select (s => AssetPersistenceManager.Shared.GetMusicAsset (s))
																				 .ToList ();
			}
			else
			{
				var newAssets = ProducerClient.Shared.AvContent [UserRoles.General].Where (m => m.HasId && m.HasStorageUrl && !allAssets.Any (ma => ma.Id == m.Id))
																				   .Select (s => AssetPersistenceManager.Shared.GetMusicAsset (s));

				allAssets.AddRange (newAssets);

				allAssets.RemoveAll (ma => !ProducerClient.Shared.AvContent [UserRoles.General].Any (a => a.Id == ma.Id));

				allAssets.Sort ((x, y) => !x.Music.UpdatedAt.HasValue ? -1 : !y.Music.UpdatedAt.HasValue ? 1 : y.Music.UpdatedAt.Value.CompareTo (x.Music.UpdatedAt.Value));
			}
		...
		void handlePersistanceManagerDidRestore (object sender, EventArgs e)
		{
			...
			ProducerClient.Shared.AvContentChanged += handleAvContentChanged;
		}
